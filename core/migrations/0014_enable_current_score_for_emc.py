...
def enable_for_emc(apps, schema_editor):
    ...
    League.objects.filter(name=LEAGUE_NAME).update(show_current_score=True)


def disable_for_emc(apps, schema_editor):
    League = apps.get_model("core", "League")
    League.objects.filter(name=LEAGUE_NAME).update(show_current_score=False)


class Migration(migrations.Migration):

    dependencies = [
        ("core", "0013_league_show_current_score"),
    ]
